...
<style>
    ...
    .stSelectbox svg { fill: #000000 !important; } /* Fix Dropdown Arrow Visibility */
...
# =========================================================
# PAGE: PRODUCTION (SIMPLIFIED & UPDATED)
# =========================================================
    ...
    with t1:
        ...
        # 1. Basic Info (SIMPLIFIED: Removed Code & Color)
        ...
        fab_sel = c3.selectbox("Fabric Used", [""] + avail_fabrics)
        fab_wt = st.number_input("Fabric Weight (Kg)", 0.0)
        
        selected_rolls = []
        if fab_sel:
            with st.expander("Pick Rolls (Optional)", expanded=False):
                # We need a color to filter rolls, assume "Any" or user selects one just for rolls
                roll_col = st.selectbox("Fabric Color (For Roll Selection)", [""] + db.get_colors())
                if roll_col:
                    rls = db.get_available_rolls(fab_sel, roll_col)
                    if rls:
                        roll_opts = [f"{r['roll_no']} ({r['quantity']}kg)" for r in rls]
                        sel_rolls_ui = st.multiselect("Select Rolls", roll_opts)
                        selected_rolls = [r['_id'] for r in rls if f"{r['roll_no']} ({r['quantity']}kg)" in sel_rolls_ui]
                    else: st.caption("No rolls available.")

        st.markdown("###### Bundles (Size & Color)")
        ...
        if st.button("🚀 Launch Lot & Generate QR", type="primary"):
            if itm and cm and st.session_state.lot_variants:
                # Pass materials as a simple list for now since UI doesn't have multi-mat yet
                mats = [{"name": fab_sel, "qty": fab_wt, "uom": "Kg"}] if fab_sel else []
                
                db.create_advanced_lot(lot_no, itm, cm, mats, st.session_state.lot_variants, fab_wt)
    ...
    with t2:
        ...
        lot_sel = st.selectbox("Select Active Lot", [""] + db.get_active_lots())
        if lot_sel:
            lot_data = db.get_lot_info(lot_sel)
            bundles = lot_data.get('bundles', [])
            
            if bundles:
                with st.expander("Move Bundles", expanded=True):
                    c1, c2, c3 = st.columns(3)
                    target_stage = c1.selectbox("Move To", db.get_all_processes())
                    worker = c2.selectbox("Assign Worker", db.get_staff("Stitching Karigar"))
                    
                    b_opts = [f"{b['bundle_id']} | {b['color']} {b['size']} ({b['current_stage']})" for b in bundles]
                    selected_b_strs = st.multiselect("Select Bundles", b_opts)
                    ...
                    if st.button("Move Selected", type="primary"):
                        if selected_b_strs and target_stage and worker:
                            ...
                            st.success(f"Moved {len(sel_ids)} bundles!")
                            st.rerun()
...
# =========================================================
    ...
    with t1:
        ...
        for l in active_lots:
            # Re-calc summary from bundles
            bundles = l.get('bundles', [])
            c = sum(b['qty'] for b in bundles if b['current_stage'] == 'Cutting')
            s = sum(b['qty'] for b in bundles if b['current_stage'] == 'Stitching')
            f = sum(b['qty'] for b in bundles if b['current_stage'] == 'Finishing')
            cut_p += c; st_p += s; fin_p += f
            summary_data.append({"Lot": l['lot_no'], "Item": l['item_name'], "Total": l['total_qty'], "Cut": c, "Stitch": s, "Finish": f})
        
        c1, c2 = st.columns(2); c1.metric("Active Lots", len(active_lots)); c2.metric("In Stitching", st_p)
    ...
    with t2:
        ...
        if l_s:
            ...
            if l:
                ...
                # Bundle List
                b_df = pd.DataFrame(l.get('bundles', []))
                if not b_df.empty:
                    render_df(b_df[['bundle_id', 'color', 'size', 'qty', 'current_stage', 'assigned_to']], file_name=f"lot_bundles_{l_s}")
                ...
                if txns:
                    ...
                    render_df(df_tx[['timestamp', 'to_stage', 'karigar', 'qty', 'bundle_ids']], file_name=f"lot_history_{l_s}")
