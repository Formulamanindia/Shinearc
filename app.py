...
<style>
    ...
    .stat-tile {
        ...
        border: 1px solid #E2E8F0; margin-bottom: 5px; box-shadow: 0 1px 2px rgba(0,0,0,0.03);
    }
    ...
    /* --- SEGMENTED CONTROL STYLING (OPTIONAL TWEAKS) --- */
    /* This targets the segmented control to ensure it fits well on mobile */
...
# --- 4. NAVIGATION (UPDATED TO SEGMENTED CONTROL) ---
# This matches the style of the Master Setup
nav_options = ["🏠 Home", "👷 Workers", "⚙️ Masters", "💰 Pay"]

# Using segmented_control for that pill-shaped look
selected_nav = st.segmented_control("Main Menu", nav_options, default="🏠 Home", label_visibility="collapsed")

# Fallback in case selection is None (on first load sometimes)
if not selected_nav:
    selected_nav = "🏠 Home"
