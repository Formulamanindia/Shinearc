...
# SUPPLIER LEDGER
    ...
    with t1:
        with st.container(border=True):
            ...
            txn_type = st.radio("Transaction Type", ["Bill (Purchase)", "Payment (Outgoing)", "Debit Note (Return)"], horizontal=True)
            st.markdown("---")
            ...
            if txn_type == "Bill (Purchase)":
                ...
                if st.session_state.bill_items:
                    ...
                    # Beautiful Table for Items in Entry
                    st.dataframe(df_items, use_container_width=True, column_config={
                        "Total": st.column_config.NumberColumn(format="₹ %.2f"),
                        "Tax Amt": st.column_config.NumberColumn(format="₹ %.2f"),
        ...
        if sel_sup:
            ...
            if v_mode == "Detailed Statement":
                ...
                if not df.empty:
                    ...
                    # PRETTY LEDGER TABLE
                    st.dataframe(df[["Date", "Type", "Reference", "Debit", "Credit", "Balance", "Remarks"]], 
                    ...
                    txn_opts = [f"{r['Date']} | {r['Type']} | ₹{r['Credit'] or r['Debit']}" for _, r in df.iterrows()]
                    sel_txn_idx = st.selectbox("Select Transaction", range(len(txn_opts)), format_func=lambda x: txn_opts[x])
                    
                    # VIEW ITEMS IF AVAILABLE
                    selected_row = df.iloc[sel_txn_idx]
                    if selected_row['Items']:
                        st.markdown("##### 🧾 Item Details for Selected Bill")
                        st.dataframe(pd.DataFrame(selected_row['Items']), use_container_width=True)
                    
                    c_del, c_edit = st.columns(2)
                    if c_del.button("🗑️ Delete Transaction"):
                        if db.delete_supplier_txn(selected_row['ID']): st.success("Deleted!"); st.rerun()
                        
                    with c_edit.expander("✏️ Edit"):
                        eid = selected_row['ID']
                        e_date = st.date_input("New Date", pd.to_datetime(selected_row['Date']))
                        val = float(selected_row['Credit'] if selected_row['Type']=='Bill' else selected_row['Debit'])
                        e_amt = st.number_input("New Amount", 0.0, value=val)
                        e_ref = st.text_input("New Reference", selected_row['Reference'])
                        e_rem = st.text_input("New Remarks", selected_row['Remarks'])
                        if st.button("Update"):
                ...
                if not df.empty:
                    ...
                    st.dataframe(df, use_container_width=True, column_config={
                        "Total Purchase": st.column_config.NumberColumn(format="₹ %.2f"),
                        "Total Outgoing": st.column_config.NumberColumn(format="₹ %.2f"),
                        "Closing Balance": st.column_config.NumberColumn(format="₹ %.2f"),
                    })
                else: st.info("No records")
...
# TRACK LOT
    ...
    if l_s:
        ...
        if l:
            ...
            # Pivot the stage stock for better readability
            stock_data = l['current_stage_stock']
            stages = list(stock_data.keys())
            
            # Collect all unique size/color keys
            all_keys = set()
            for s in stages:
                all_keys.update(stock_data[s].keys())
            ...
            if txns:
                t_df = pd.DataFrame(txns)[['timestamp', 'from_stage', 'to_stage', 'karigar', 'variant', 'qty']]
