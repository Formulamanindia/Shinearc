...
if page == "Dashboard":
    ...
    with c1:
        with st.container(border=True):
            ...
            st.markdown('<div class="dash-card-sub">Production Line</div>', unsafe_allow_html=True)
    with c2:
        with st.container(border=True):
...
# MASTERS
    ...
    with t4: # ITEM MASTER WITH 5 FABRIC OPTIONS
        with st.container(border=True):
...
# CUTTING FLOOR (MULTI-FABRIC SUPPORT)
    ...
    with st.container(border=True):
        ...
        if not required_fabrics:
            st.info("Select an Item to load required fabrics.")
        else:
        ...
        if st.session_state.lot_breakdown:
            ...
            if st.button("🚀 CREATE LOT", type="primary"):
                ...
                if missing_fabs:
                    ...
                    db.create_lot({
                        "lot_no": next_lot, "item_name": inm, "item_code": icd, 
                        "color": acol, "created_by": cut, "size_breakdown": st.session_state.lot_breakdown, 
                        "fabrics_consumed": fab_summary_list, "total_fabric_weight": total_weight_all
                    }, flat_roll_ids)
...
# INVENTORY
    ...
    with t3:
        ...
        with c2:
            with st.container(border=True):
                st.markdown("**Outward**")
                ao = st.selectbox("Item", [""] + db.get_accessory_names(), key="out")
                qo = st.number_input("Qty Out", 0.0)
                if st.button("Issue"):
                    if ao and qo>0: db.update_accessory_stock(ao, "Outward", qo, "N/A"); st.success("Issued!"); st.rerun()
        st.divider()
        st.dataframe(pd.DataFrame(db.get_accessory_stock()))
