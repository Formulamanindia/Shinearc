...
if nav == "📊 Dashboard":
    ...
    with st.container(key="mobile_grid_metrics"):
        ...
        with m1:
            render_metric_card("Pieces Today", f"{pcs:,.0f}", "👕", "#D1FAE5", "#10B981")
        with m2:
            render_metric_card("Prod Value", f"₹{earn:,.0f}", "₹", "#FEF3C7", "#F59E0B")
        with m3:
            render_metric_card("Liabilities", f"₹{pending:,.0f}", "💳", "#FEE2E2", "#EF4444")
        with m4:
            render_metric_card("Active Staff", f"{active}", "👥", "#DBEAFE", "#3B82F6")
    ...
    with tab_cut:
        ...
        if act == "Create New Lot":
            ...
            if c_b1.button("🔄 Generate Grid", use_container_width=True):
                ...
                if not l_no or not item_name:
                    ...
                    if s: 
                        ...
                        if 'lot_df' in st.session_state:
                            del st.session_state['lot_df']
                        if 'fab_df' in st.session_state:
                            del st.session_state['fab_df']
                        time.sleep(1)
                        st.rerun()
                    else:
                        st.error(m)
            ...
            with st.form("stitch_log"):
                ...
                if st.form_submit_button("💾 Submit Entry & Credit Ledger", type="primary", use_container_width=True):
                    if sd_worker and sd_lot and sd_bun:
                        ...
                        if s:
                            st.success(f"Success! Credited to ledger: ₹{qty*fin_rate:,.2f}")
                        else:
                            st.error(m)
                    else:
                        st.error("Please fill in all required fields (Worker, Lot, Bundle).")
    ...
    <a href="{prod.get('url', '#')}" target="_blank" class="product-link-btn">View Reference Product</a>
                            ...
                            with btn_c2.popover("⚙️ Edit", use_container_width=True):
    ...
    if not channels:
                            ...
                            with st.container(key=f"mobile_grid_cogs_{c_name}"):
                                ...
                                with cogs_c1:
                                    if o_sku != "Select File Column...":
                                        ...
                                        df_template = pd.DataFrame({o_sku: unique_skus, 'COGS': 0.0})
                                        csv_template = df_template.to_csv(index=False).encode('utf-8')
                                        st.download_button(f"⬇️ Download SKU Template ({len(unique_skus)})", csv_template, f"{c_name}_COGS.csv", "text/csv", use_container_width=True)
                                    else:
                                        st.info("Map 'SKU' to generate template.")
                                ...
                                if "Select File Column..." not in [o_dt, o_sku, o_am]:
                                    try:
                                        ...
                                        if o_stat != "Select File Column...":
                                            ...
                                            rto_counts = df_o[df_o['StatusClean'].str.contains('rto', na=False)].groupby(o_sku).size()
                                            
                                            sku_stats['Delivered'] = sku_stats[o_sku].map(deliv_counts).fillna(0)
                                            sku_stats['Cancelled'] = sku_stats[o_sku].map(canc_counts).fillna(0)
                                            sku_stats['Cust_Return'] = sku_stats[o_sku].map(ret_counts).fillna(0)
                                            sku_stats['RTO'] = sku_stats[o_sku].map(rto_counts).fillna(0)
                                        ...
                                        if has_cogs:
                                            ...
                                            for c in ['Sale_Value', 'Rate', 'Cost_of_Goods', 'Net_After_Cost']:
                                                prof_stats[c] = prof_stats[c].apply(lambda x: f"₹{x:,.2f}")
                                    ...
                                    st.warning("Please map Date, SKU, and Amount.")
        ...
        with doc_tabs[0]:
            ...
            with c1:
                with st.container(border=True):
                    ...
                    if st.button("Generate RTO Heatmap & Blocklist", type="primary", use_container_width=True):
                        if rto_file:
                            st.success("Analysis complete! 15 High-Risk Pincodes identified for COD blocking.")
                        else:
            ...
            with c2:
                with st.container(border=True):
                    ...
                    if st.button("Link Returns to Production Lots", type="primary", use_container_width=True):
                        if ret_file:
                            st.success("Successfully mapped! Spike in 'Size Issue' detected in Lot L-1002.")
                        else:
                            st.warning("Please upload a report first.")
            ...
            with st.container(border=True):
                ...
                if st.button("Analyze Inventory Aging", type="primary", use_container_width=True):
                    if inv_file:
                        st.success("Analysis complete! 3 SKUs identified with >90 days of cover. Recommended action: Liquidate.")
                    else:
                        st.warning("Please upload a report first.")
        ...
        with doc_tabs[1]:
            ...
            with c1:
                with st.container(border=True):
                    ...
                    if st.button("Generate Alerts", type="primary", use_container_width=True):
                        if vel_file:
                            st.success("Calculated! 2 SKUs require immediate Cutting Lots.")
                        else:
                            st.warning("Upload report.")
            with c2:
                with st.container(border=True):
            ...
            with st.container(border=True):
                ...
                if st.button("Generate ROAS Quadrant Matrix", type="primary", use_container_width=True):
                    if ad_spend and pl_data:
                        st.success("Matrix Generated! 4 SKUs moved to 'Scale' quadrant.")
                    else:
        ...
        with tab2:
            ...
            if not df_comp.empty:
                ...
                st.warning("No GST clients registered.")
        with tab3:
        ...
        with tab4:
            ...
            if calc_mode == "Manual Summary":
                ...
                with st.container(border=True):
                    ...
                    st.caption("Upload raw Flipkart Sales and Cashback reports to extract and map exact tax liabilities.")
                    ...
                    if st.button("🚀 Calculate Flipkart Liability", type="primary", use_container_width=True):
                        if sales_file and cb_file:
                            ...
                            st.warning("Upload both Sales and Cashback reports to proceed.")
        ...
        with t1:
            ...
            with st.form("cf"):
                ...
                if parents:
                    ...
                    st.info("Create Parent first.")
                    st.form_submit_button("Create Variant", disabled=True)
        with t2:
            ...
            if st.button("⚠️ CONFIRM TRUNCATE", type="primary", use_container_width=True):
                if selected_wipe:
                    ...
                    for s in selected_wipe:
                        cols.extend(wipe_opts[s])
                    db.clean_database(cols)
                    st.success("Truncated!")
                    st.rerun()
