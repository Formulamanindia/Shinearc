...
def image_to_base64(f):
    try: return f"data:{f.type};base64,{base64.b64encode(f.getvalue()).decode()}"
    except: return ""
        ...
        if act=='delete': 
            ...
            if exists:
                ...
                for k,v in row.items(): 
                    if k in ['mrp','selling_price','gst_rate']: upd[k]=safe_float(v)
                    elif k=='stock': upd[k]=safe_int(v)
                    else: upd[k]=str(v)
            ...
            for sz in vars_:
                ...
                if db.catalog.find_one({"sku":fsku}): errors.append({"Row":i+2,"SKU":fsku,"Error":"Exists"}); continue
                doc={**row.to_dict(), "sku":fsku, "group_id":gid, "sort_index":sidx, "image_link_1":img, "variation":sz, "mrp":safe_float(row.get('mrp')), "selling_price":safe_float(row.get('selling_price')), "stock":safe_int(row.get('stock')), "last_updated":datetime.datetime.now()}
...
def generate_marketplace_file(p):
    df=get_catalog_df()
    if df.empty: return None
    out=pd.DataFrame(); out['SKU']=df['sku']; out['Name']=df.get('product_name',''); out['MRP']=df.get('mrp',0); out['SP']=df.get('selling_price',0); return out
