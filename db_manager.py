...
# ==========================================
# 1. DATABASE UTILITIES (NEW CLEANER)
# ==========================================
def clean_database(collections):
    """
    Wipes selected collections from the database.
    collections: list of collection names strings
    """
    try:
        for col in collections:
            db[col].delete_many({})
        return True, "Selected data cleared successfully."
    except Exception as e:
        return False, str(e)

# ==========================================
# 2. PRODUCTION & LOTS (ENHANCED)
# ==========================================
def get_item_fabrics(item_name):
    """Fetches fabrics linked to an item name via its code."""
    # Find item code first if needed, or just search by name if structure allows
    # Assuming Items collection has: item_name, fabrics (list)
    item = db.items.find_one({"item_name": item_name})
    if item and 'fabrics' in item:
        return item['fabrics']
    return []
...
def create_lot(lot_no, item_name, item_code, color, size_breakdown, rolls, cutting_master, fabric_weight):
    """Creates a production lot with enhanced details."""
    ...
    db.lots.insert_one({
        ...
        "item_code": item_code,
        "color": color,
        "fabric_weight": fabric_weight, # New Field
        "total_qty": total_qty,
...
def get_staff_payout(staff_name, month, year):
    ...
    adv_pipeline = [{"$match": {"staff": staff_name, "type": "Advance", "date": {"$gte": start, "$lt": end}}}, {"$group": {"_id": None, "total": {"$sum": "$amount"}}}]
...
# ==========================================
# 4. BULK UPLOAD & HELPERS (Preserved)
# ==========================================
...
def get_gst_slabs(): slabs = list(db.gst_slabs.find({}, {"_id":0, "rate":1})); return [s['rate'] for s in slabs] if slabs else [0, 5, 12, 18, 28]
